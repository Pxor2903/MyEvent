...
export const Home: React.FC<HomeProps> = ({ user, onLogout }) => {
  ...
  const renderDashboard = () => (
    <div className="space-y-8">
      ...
      {showJoinModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="join-title">
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50" aria-label="Navigation">
        <div className="max-w-4xl mx-auto px-4 h-14 flex justify-between items-center">
          <button type="button" onClick={() => setView('dashboard')} className="focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-lg" aria-label="Accueil">
            <Logo />
          </button>
          <div className="flex items-center gap-3">
            <span className="text-sm text-slate-600 hidden sm:inline">Bonjour, {user.firstName}</span>
            <img src={`https://ui-avatars.com/api/?name=${user.firstName}+${user.lastName}&background=6366f1&color=fff`} className="w-8 h-8 rounded-lg" alt="" />
            <button type="button" onClick={onLogout} className="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50" aria-label="Déconnexion">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-6 sm:py-8">
        {view === 'dashboard' && (
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <h1 className="text-2xl font-semibold text-slate-900">Vos événements</h1>
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowJoinModal(true)} className="px-4 py-2.5 border border-slate-200 text-slate-700 text-sm font-medium rounded-xl hover:bg-slate-50">
                Rejoindre
              </button>
              <button type="button" onClick={() => setView('create-event')} className="px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-xl hover:bg-indigo-700 flex items-center gap-2">
