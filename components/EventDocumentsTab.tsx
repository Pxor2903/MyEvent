...
export const EventDocumentsTab: React.FC<EventDocumentsTabProps> = ({
  ...
  /** Index dans la file WhatsApp (null = pas en cours). Permet d’ouvrir toutes les conv WhatsApp une par un. */
  ...
  useEffect(() => {
    if (!shareDoc) setWhatsAppQueueIndex(null);
  }, [shareDoc]);
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(() => {
                      ...
                      if (withWhatsApp.length > 0) {
                        const inQueue = whatsAppQueueIndex !== null;
                        const current = inQueue ? withWhatsApp[whatsAppQueueIndex!] : null;
                        return (
                          <div className="flex flex-col gap-2 w-full">
                            {!inQueue ? (
                              <button
                                type="button"
                                onClick={() => setWhatsAppQueueIndex(0)}
                              ...
                              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                                <p className="text-sm font-medium text-slate-700">
                                  Conversation {whatsAppQueueIndex! + 1} sur {withWhatsApp.length} – {current.guest.firstName} {current.guest.lastName}
                                </p>
                                <div className="flex gap-2">
                                  <button
                                    type="button"
                                    onClick={() => { if (current.whatsappUrl) window.open(current.whatsappUrl, '_blank', 'noopener,noreferrer'); }}
                                    className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#25D366] text-white text-sm font-medium hover:opacity-90"
                                  >
                                    Ouvrir WhatsApp
                                  </button>
                                  <button
                                    type="button"
                                    onClick={() => {
                                      if (whatsAppQueueIndex! >= withWhatsApp.length - 1) setWhatsAppQueueIndex(null);
                                      else setWhatsAppQueueIndex(whatsAppQueueIndex! + 1);
                                    }}
                                    className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 text-sm font-medium hover:bg-slate-50"
                                  >
                                    {whatsAppQueueIndex! >= withWhatsApp.length - 1 ? 'Terminer' : 'Suivant'}
                                  </button>
                                </div>
                              </div>
                            ) : null}
