                ...
                {canManage && guestsForSub.length > 0 && (
                  <>
                    <button
                      ...
                      className="px-3 py-2 rounded-lg border border-sky-200 bg-sky-50 text-sky-700 text-sm font-medium hover:bg-sky-100"
                    >
                      Envoyer à tous
                    </button>
                    <button
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex flex-col gap-4">
                        ...
                        {/* 3. Envoyer à tous en un clic par SMS (API Twilio) — toujours affiché si des invités ont un numéro */}
                        {(() => {
                          const withSms = shareData.entries.filter(e => e.canShare && e.smsUrl);
                          const phoneNumbersSms = withSms.map((e) => toE164((e.guest.phone ?? '').trim())).filter((n) => n);
                          const withSmsPending = withSms.filter((e) => e.guest.status === 'pending');
                          const phoneNumbersSmsPending = withSmsPending.map((e) =>
                            toE164((e.guest.phone ?? '').trim())
                          ).filter((n) => n);
                          const smsApiConfigured = isSmsApiConfigured();
                          if (phoneNumbersSms.length === 0) return null;
                          return (
                            <div className="flex flex-col gap-2">
                              <p className="text-sm font-medium text-slate-700">
                                Envoyer à tous en un clic (SMS)
                              </p>
                              <p className="text-xs text-slate-500">
                                {smsApiConfigured
                                  ? 'Message groupé envoyé à tous les invités par SMS en une fois (API Twilio). Aucune confirmation manuelle.'
                                  : 'Configurez l’API SMS sur Vercel (VITE_SMS_API_URL + TWILIO_SMS_FROM) pour activer l’envoi à tous en un clic. Voir docs/SMS_ENVOI_GROUPE.md.'}
                              </p>
                              <button
                                type="button"
                                disabled={smsSending || !smsApiConfigured}
                                onClick={async () => {
                                  if (!smsApiConfigured) return;
                                  setSmsResult(null);
                                  setSmsSending(true);
                                  try {
                                    const baseMessage = shareMessage;
                              ...
                              >
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                                {smsSending ? 'Envoi en cours…' : `Envoyer à tous par SMS (${phoneNumbersSms.length} invité${phoneNumbersSms.length > 1 ? 's' : ''})`}
                              </button>
                              {withSmsPending.length > 0 && (
                                <button
                                  type="button"
                                  disabled={smsSending || !smsApiConfigured}
                                  onClick={async () => {
                                    if (!smsApiConfigured) return;
                                    if (phoneNumbersSmsPending.length === 0) return;
                                    setSmsResult(null);
                                    setSmsSending(true);
                                    try {
                                      const baseMessage = shareMessage;
                                      ...
                                      let messagesOpt: string[] | undefined;
                                      if (isInvitationDoc(shareDoc)) {
                                        const guestIds = withSmsPending.map((e) => e.guest.id);
                                        const baseUrl =
                                          typeof window !== 'undefined' ? window.location.origin : '';
                                        if (baseUrl) {
                                          try {
                                            const links = await getOrCreateInvitationLinks(
                                              event.id,
                                              guestIds,
                                              baseUrl
                                            );
                                            if (Object.keys(links).length > 0) {
                                              messagesOpt = withSmsPending.map(
                                                (e) =>
                                                  baseMessage +
                                            ...
                                            alert(
                                              err instanceof Error
                                                ? err.message
                                                : 'Liens d’invitation indisponibles.'
                                            );
                                            setSmsSending(false);
                                            return;
                                          }
                                      ...
                                      const result = await sendSmsToMany(
                                        phoneNumbersSmsPending,
                                        messageToSend,
                                        messagesOpt
                                      );
                                      if (result.ok) {
                                        setSmsResult({
                                          sent: result.sent ?? phoneNumbersSmsPending.length,
                                          failed: result.failed,
                                          error: result.error
                                        });
                                      } else {
                                  ...
                                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-sky-50 text-sky-700 text-sm font-semibold hover:bg-sky-100 disabled:opacity-60 border border-sky-200"
                                >
                                  <svg
                                    className="w-4 h-4"
                                    fill="none"
                                  ...
                                  >
                                    <path
                                      ...
                                      strokeWidth={2}
                                      d="M8 10h.01M12 10h.01M16 10h.01M9 21h6a2 2 0 002-2v-1a9 9 0 10-10 0v1a2 2 0 002 2z"
                                    />
                                  </svg>
                                  {smsSending
                                    ? 'Relance en cours…'
                                    : `Relancer les invités sans réponse (${withSmsPending.length})`}
                                </button>
                              )}
                              {smsResult && (
                        ...
                        {/* 4. Envoyer à tous en un clic via WhatsApp (optionnel, config Twilio) — toujours affiché si des invités ont un numéro */}
                        {phoneNumbers.length > 0 && (
                          <div className="flex flex-col gap-2">
                            <p className="text-sm font-medium text-slate-700">
                              Envoyer à tous en un clic (WhatsApp)
                            </p>
                            <p className="text-xs text-slate-500">
                              {apiConfigured
                                ? 'Envoi groupé sans ouvrir chaque conversation (API Twilio). En mode sandbox, les destinataires doivent avoir rejoint le sandbox.'
                                : 'Configurez l’API WhatsApp sur Vercel (VITE_WHATSAPP_API_URL + TWILIO_WHATSAPP_FROM) pour activer l’envoi à tous. Voir docs/WHATSAPP_ENVOI_GROUPE.md.'}
                            </p>
                            <button
                              type="button"
                              disabled={whatsAppSending || !apiConfigured}
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  let messagesOpt: string[] | undefined;
                                  if (isInvitationDoc(shareDoc)) {
                                    const withWhatsAppEntries = shareData.entries.filter(
                                      (e) => e.canShare && e.whatsappUrl
                                    );
                            ...
                            >
                              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                              {whatsAppSending ? 'Envoi en cours…' : `Envoyer à tous par WhatsApp (${phoneNumbers.length} invité${phoneNumbers.length > 1 ? 's' : ''})`}
                            </button>
                            {(() => {
                              const withWhatsAppEntries = shareData.entries.filter(
                                (e) => e.canShare && e.whatsappUrl
                              );
                              const withWhatsAppPending = withWhatsAppEntries.filter(
                                (e) => e.guest.status === 'pending'
                              );
                              const pendingNumbers = withWhatsAppPending
                                .map((e) => toE164((e.guest.phone ?? '').trim()))
                                .filter((n) => n);
                              if (withWhatsAppPending.length === 0 || pendingNumbers.length === 0)
                                return null;
                              return (
                                <button
                                  type="button"
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      ...
                                      let messagesOpt: string[] | undefined;
                                      if (isInvitationDoc(shareDoc)) {
                                        const guestIds = withWhatsAppPending.map(
                                          (e) => e.guest.id
                                        );
                                        const baseUrl =
                                        ...
                                        if (baseUrl) {
                                          try {
                                            ...
                                            if (Object.keys(links).length > 0) {
                                              messagesOpt = withWhatsAppPending.map(
                                                (e) =>
                                                  baseMessage +
                                                  `\n\nRépondre à l'invitation : ${
                                                    links[e.guest.id] ?? ''
                                      ...
                                      if (result.ok) {
                                        setWhatsAppResult({
                                          sent: result.sent ?? pendingNumbers.length,
                                          failed: result.failed,
                                          error: result.error
                                        });
                                      } else {
                                  ...
                                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-white text-[#25D366] text-sm font-semibold hover:bg-[#25D366]/5 disabled:opacity-60 border border-[#25D366]/60"
                                >
                                  <svg
                                    className="w-4 h-4"
                                    fill="currentColor"
                                  ...
                                  {whatsAppSending
                                    ? 'Relance en cours…'
                                    : `Relancer les invités sans réponse (${withWhatsAppPending.length})`}
                                </button>
                              );
                            })()}
                            {whatsAppResult && (
