              ...
              {shareData.entries.filter(e => e.canShare).length > 0 && (
                <>
                      ...
                      if (withWhatsApp.length > 0) {
                        return (
                          <button
                            type="button"
                            onClick={() => {
                              withWhatsApp.forEach((entry) => {
                                if (entry.whatsappUrl) window.open(entry.whatsappUrl, '_blank', 'noopener,noreferrer');
                              });
                            }}
                  ...
                  {shareData.entries.some(e => e.canShare && e.whatsappUrl) && (
                    <p className="w-full text-xs text-slate-500 mt-1">Un clic ouvre une conversation WhatsApp par invité. Si certaines ne s’ouvrent pas, autorisez les fenêtres pop-up ou utilisez la liste ci-dessous.</p>
                  )}
                    {(() => {
