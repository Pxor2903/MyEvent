        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          {suggestions.map((s, i) => (
            <button
              key={i}
              type="button"
              className="w-full text-left px-4 py-3 hover:bg-teal-50 transition-colors border-b last:border-0 border-gray-100 flex flex-col group"
              onClick={() => handleSelect(s)}
            >
              <span className="font-semibold text-gray-900 group-hover:text-teal-600 text-sm transition-colors">{s.properties.label}</span>
